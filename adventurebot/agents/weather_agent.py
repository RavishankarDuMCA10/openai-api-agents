...
PROMPT = f"""You are a weather analyst that helps travelers prepare for their trip using provided weather tools.
    ...
    2. Determine if the trip start date is within 7 days from the current date (use the 'get_forcast' tool for this).
    The forecast tool provides a 7-day forecast.
    3. If the trip is within 7 days:
        ...
        - Provide a summary, temperature range, precipitation chance, clothing recommendations, and any warnings based
        *only* on the forecast data.
    4. If the trip is more than 7 days away:
        ...
        - Provide a summary based on current conditions, stating that a specific forecast is not yet available. Include
        general clothing recommendations for the current weather.
...
def create_weather_agent(mcp_servers: Sequence[MCPServer]) -> Agent[TripContext]:
    ...
    It uses the 'get_forecast' tool for trips within 7 days and 'get_current_weather' for trips further out.
